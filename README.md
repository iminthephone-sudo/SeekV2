...
| **AI coaching (spaCy)** | Scores the resume and reviews every bullet (action verb? numbers? weak opener?). Suggests skills the experience already demonstrates. **Write with spaCy** drafts the professional summary from the profile (optionally aimed at a posting) and **Improve duties** rewrites job-duty bullets (action verb, right tense, a [#] prompt where a number belongs) and suggests typical duties for the job title. |
| **Job search** | Search LinkedIn and Indeed from the Jobs page by keywords and location, optionally only fair-chance postings, then import any result with a double-click. |
| **Job-site sign-ins** | Each profile can be signed in to LinkedIn and Indeed through SEEK's built-in browser (on the site's own page — SEEK never sees the password), so imports and searches run as that participant. |
...
| **Interview practice** | **S.T.A.R.S** (Situation, Task, Action, Result, Skills) answers for 12 behavioral questions, or your own. Includes story ideas pulled from the profile and part-by-part spaCy coaching ("I" vs "we", concrete steps, numbers, filler words, blame words, skills tied to the posting). Produces a polished answer to rehearse. **Talking about your record** practises the criminal-history questions ("Have you ever been convicted…?", "Why should we trust you…?", parole scheduling, the gap) with coaching on the four moves — answer honestly, own it, show change, turn to the job — flags for blame, minimizing, oversharing and jail slang, proof of change pulled from the profile, and a know-your-rights note. **Workplace assessment** practice has 20 agree/disagree statements like those in hourly-job applications, with coaching as you answer, trait bars, and flags for contradictions and too-good-to-be-true answers. |
